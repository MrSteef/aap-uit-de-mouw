...
/// fuzz-testing opponent and as a stand-in bot.
pub struct RandomAgent;

impl PlayerAgent for RandomAgent {
    /// Panics if `legal` is empty — which `GameEngine::legal_actions`
    /// should never actually produce, since it always falls back to
    /// `TurnAction::Pass` when nothing else is available
    /// (`RuleConfig::no_available_action_behavior`). Kept as a defensive
    /// check rather than removed outright.
