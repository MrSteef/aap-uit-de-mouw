...
pub struct RuleConfig {
    ...
    /// to its recent history into the pile, as a cost of completing its
    /// journey.
    pub finished_pawn_dumps_history_destination: bool,
    /// Whether a big caught lie's spoils are split (the directly-challenged
    /// move to the challenger, everything swept up along with it to the
    /// pile) rather than all going to the challenger.
...
pub(crate) fn minimal_rules() -> RuleConfig {
    RuleConfig::builder()
        ...
        .automatic_audit_reward_destination(true)
        .finished_pawn_dumps_history_destination(true)
        .cascade_lie_rewards_destination(true)
